...
梯度下降对原理也是基于函数在极值点导数为0，找到导数为0对点就可以了。那怎么找呢？

我们先随机猜测一个值 $$x_0$$ ，检查 $$f'(x_0)$$ ，如果小于0，说明在这个点处于下降对趋势，最小值在右侧，把 $$x_0$$ 加上一个值 $$\delta$$ 继续验证 $$f'(x_0+\delta)$$，假如大于0，在这个点处于上升趋势，最小值在左侧，那就在 $$x_0$$ 减去一个值 $$\delta$$继续验证 $$f'(x_0-\delta)$$。离最小值越近，我们要增加或减少对值应该越小，因为这样才能无限逼近最小值，所以更新的值与导数有关。所以更新策略如下：
...
这个也称为L2 Penalty，而如果加的是系数的和而不是平房和，则称为L1 Penalty。而 $$\lambda$$ 又是一个类似于学习率的超参数，可以调节惩罚，调节过拟合与欠拟合。
...
还有归一话，例如我们见的像素值范围是0-255，那么我们把所有值都除以255。
